...
```json
    "dev": "AI_GATEWAY_API_KEY=$(op read 'op://Private/vercel ai api-gateway key/password' --account my.1password.com) TAVILY_API_KEY=$(op read 'op://Private/tavily api key free tier/password' --account my.1password.com) tsx src/index.ts",
    "dev:full": "AI_GATEWAY_API_KEY=$(op read 'op://Private/vercel ai api-gateway key/password' --account my.1password.com) TAVILY_API_KEY=$(op read 'op://Private/tavily api key free tier/password' --account my.1password.com) tsx src/index.ts -- --keyboards-mcp ../keyboards-mcp/dist/index.js --audio-mcp ../audio-analysis-mcp/.venv/bin/python",
```

- [ ] **Step 2: Verify the 1Password path resolves**

Run: `op read 'op://Private/tavily api key free tier/password' --account my.1password.com | head -c 8`
